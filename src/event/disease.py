...
# level 2: unknown serotype
  ...
  def __init__(self, d_serotype, d_subtype, d_type, d_pathogenicity=None):
    ...
    if self.pathogenicity is None:
      if self.get_max_hierarchy_level() == len(self.get_entry()): # i.e. self.d_serotype is not empty
  ...
  def get_value_at_hierarchy_level(self, hierarchy_level):
    entry = self.get_entry() # example: ["h5n1", "h5 or h7 subtypes", "avian influenza"] (more specific to more general)
  ...
  def is_hierarchically_included(self, another_dis):
    #print("self entry", self.get_entry())
    #print("other entry", another_dis.get_entry())
