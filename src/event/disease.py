...
class Disease:
  ...
  def is_hierarchically_included(self, another_dis):
    if ("unknown" in another_dis.d_type and "unknown" not in self.d_type):
      return True
    elif ("unknown" not in self.d_type) and ("unknown" not in another_dis.d_type) and (self.d_type != another_dis.d_type):
      return False
  ...
  def hierarchically_includes(self, another_dis):
    if ("unknown" in self.d_type and "unknown" not in another_dis.d_type):
      return True
    elif ("unknown" not in self.d_type) and ("unknown" not in another_dis.d_type) and (self.d_type != another_dis.d_type):
      return False
    #if self.d_type == another_dis.d_type:
