...
class Host:
  ...
  def __init__(self, dict_data = None):
    ...
    if dict_data is not None:
      ...
      self.hierarchy_level = self.dict_depth(self.dict_data)
  ...
  def get_unnested_entry(self):
    return(unnest_dict(self.dict_data))
  ...
  def merge_with_other_dict_data(self, other_dict_data):
    ...
    #   return result
    other_tuples = unnest_dict(other_dict_data)
    final_other_tuples = []
  ...
  def is_hierarchically_included(self, another_host):
    h1 = list(self.dict_data.keys())[0]
    h2 = list(another_host.dict_data.keys())[0]
    if ("unknown" not in h1 and "unknown" in h2):
      return True
    elif ("unknown" not in h1) and ("unknown" not in h2) and (h1 != h2):
      return False
    #
  ...
  def hierarchically_includes(self, another_host):
    h1 = list(self.dict_data.keys())[0]
    h2 = list(another_host.dict_data.keys())[0]
    if ("unknown" in h1 and "unknown" not in h2):
      return True
    elif ("unknown" not in h1) and ("unknown" not in h2) and (h1 != h2):
      return False
    #
  ...
  def is_identical(self, another_dis):
    ...
    if self.hierarchy_level != another_dis.hierarchy_level:
      return False
    temp = unnest_dict(self.get_entry())
    other_temp = unnest_dict(another_dis.get_entry())
